...
import type { Json, WikiEntity, WikiEntityType } from "@/types/database.types";
...
export async function getWikiEntityWithRelations(slug: string) {
  ...
  const [relationsAResult, relationsBResult, storyRelationsResult] =
    await Promise.all([
      // Get relations where this entity is entity_a
      supabase
        .from("entity_relations")
        .select(
          "*, entity_b:wiki_entities!entity_relations_entity_b_id_fkey(id, name, slug, entity_type, image_url)",
        )
      ...
      supabase
        .from("entity_relations")
        .select(
          "*, entity_a:wiki_entities!entity_relations_entity_a_id_fkey(id, name, slug, entity_type, image_url)",
        )
  ...
  return {
    ...entity,
    relatedEntities: [
      ...relationsAResult.data.map((r) => ({
        ...r.entity_b,
        relationType: r.relation_type,
        description: r.description,
      })),
      ...relationsBResult.data.map((r) => ({
        ...r.entity_a,
        relationType: r.relation_type,
        description: r.description,
      })),
    ],
    stories: storyRelationsResult.data.map((r) => ({
      ...r.story,
      relationType: r.relation_type,
    })),
  };
